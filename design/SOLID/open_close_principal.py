...
class SystemMonitor:
    ...
    def identify_event(self):
        if (
            self.event_data["before"].get("session") == 0
            and self.event_data["after"].get("session") == 1
        ):
            return LoginEvent(self.event_data)

        elif (
            self.event_data["before"].get("session") == 1
            and self.event_data["after"].get("session") == 0
        ):
...
class LoginEvent(Event):
    ...
    def meet_condition(self):
        return (
            self._raw_data["before"].get("session") == 0
            and self._raw_data["after"].get("session") == 1
        )
